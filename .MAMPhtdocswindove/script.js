...
/* harmony export */   createExtraData: () => (/* binding */ createExtraData),
/* harmony export */   "default": () => (__WEBPACK_DEFAULT_EXPORT__)
/* harmony export */ });
/* harmony import */ var _services_services__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ../services/services */ "./src/js/services/services.js");

const calc = () => {
  const balconIcons = document.querySelectorAll('.balcon_icons .balcon_icons_img'),
    bigImg = document.querySelectorAll('.big_img > img'),
    width = document.querySelector('#width'),
    heigth = document.querySelector('#height'),
    checkboxTemperature = document.querySelectorAll('.checkbox');
  ...
  validateInput(width);
  validateInput(heigth);
  checkboxTemperature.forEach(item => {
    item.addEventListener('click', () => {
      checkboxTemperature.forEach(elem => {
        elem.checked = false;
      });
      item.checked = true;
    });
  });
  checkboxTemperature[0].checked = true;
  document.querySelector('.popup_calc_profile_button').addEventListener('click', () => {
    let tempe;
    checkboxTemperature.forEach(item => {
      if (item.checked) {
        tempe = item.nextElementSibling.id;
      }
    });
    const data = {
      width: width.value,
    ...
    console.log(json, 'calc');
    (0,_services_services__WEBPACK_IMPORTED_MODULE_0__.postData)('https://jsonplaceholder.typicode.com/posts');

    // return data;
  });
};
const createExtraData = extraData => {
...
const forms = (extraData = {}) => {
  ...
  const phones = document.getElementsByName('user_phone');
  phones.forEach(item => {
    let a = '';
  ...
  forms.forEach(form => {
    ...
    form.addEventListener('submit', e => {
      ...
      (0,_services_services__WEBPACK_IMPORTED_MODULE_0__.postData)('https://jsonplaceholder.typicode.com/posts', json).then(() => div.textContent = message.sent).catch(() => div.textContent = message.error).finally(() => {
        form.reset();
...
const modal = () => {
  ...
  bindModal('.popup_calc_btn', '.popup_calc', ['.popup_calc_close', '.popup_calc_button']);
...
const timer = () => {
  ...
  function updataTimer() {
    ...
    document.querySelector('#days').textContent = getZero(days), document.querySelector('#hours').textContent = getZero(hours), document.querySelector('#minutes').textContent = getZero(minutes), document.querySelector('#seconds').textContent = getZero(seconds);
  }
  updataTimer();
